...
import { Cpu, Trophy, TrendingUp, Target, BarChart2 } from "lucide-react";
...
function DashboardHome() {
  ...
  const highestScore = Math.max(
    ...data.interviews.map((i) => i.overallScore || 0),
    0,
  );

  const excellent = data.interviews.filter((i) => i.overallScore >= 8).length;

  const good = data.interviews.filter(
    (i) => i.overallScore >= 6 && i.overallScore < 8,
  ).length;

  const needsWork = data.interviews.filter((i) => i.overallScore < 6).length;
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-5">
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm lg:text-base">
              ...
              <span className="text-blue-400 font-semibold">24 times</span> —
              keep the momentum.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-5">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 text-white text-sm font-semibold rounded-xl shadow-lg shadow-blue-500/25 hover:-translate-y-0.5 hover:shadow-xl transition-all duration-200"
              >
                <Cpu size={16} />
                Start New Interview
              </button>
        ...
        {stats.map((item) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-slate-400 text- mt-1">{item.title}</p>
            </div>
          </div>
        ))}
      </div>
